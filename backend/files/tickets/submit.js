...
export default async function SubmitTicket(req,res){
    ...
    try{
        ...
        console.log(p);
        var m=null;
        const{category_id,priority_id,user_id,ticket_issue}=req.body;
        console.log(category_id)
        console.log(priority_id)
        console.log(user_id)
        if(!category_id||!priority_id||!user_id||!ticket_issue){
            console.log("all fields enter")
            return res.status(409).json({message:"Enter all fields"});
        }
        var staff_id=null
        await con.query(`select count(t.staff_id),t.staff_id as tickets_staff,r.staff_id as register_staff,c.category_name from kemfri_schema.register r
